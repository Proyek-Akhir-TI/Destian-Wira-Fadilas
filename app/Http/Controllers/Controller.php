...
class Controller extends BaseController
{
	...
	protected $rajaOngkirBaseUri = null;
	...
	public function __construct() {
		...
		$this->rajaOngkirBaseUri = env('RAJAONGKIR_BASE_URI');
    ...
    protected function rajaOngkirRequest($resource, $params = [], $method = 'GET')
	{
		...
		$url = $this->rajaOngkirBaseUri . $resource;
		if ($params && $method == 'POST') {
			$requestParams['form_params'] = $params;
		} else if ($params && $method == 'GET') {
			$query = is_array($params) ? '?'.http_build_query($params) : '';
			$url = $this->rajaOngkirBaseUri . $resource . $query;
		}
